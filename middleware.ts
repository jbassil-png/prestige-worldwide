...
export async function middleware(request: NextRequest) {
  ...
  if (!user && (pathname.startsWith("/dashboard") || pathname.startsWith("/dev") || pathname.startsWith("/onboarding"))) {
  ...
  // Redirect authenticated users away from sign-in (but not sign-up — always accessible)
...
export const config = {
  matcher: ["/dashboard/:path*", "/onboarding/:path*", "/sign-in", "/sign-up", "/dev/:path*"],
};
